...
const PORTS = [appConfig.cdpPort || 9000, 9001, 9002, 9003];
const POLL_INTERVAL = 1000; // 1 second
const SERVER_PORT = appConfig.port || process.env.PORT || 3001;
const APP_PASSWORD = appConfig.password || process.env.APP_PASSWORD || 'antigravity';
const AUTH_COOKIE_NAME = 'ag_auth_token';
let AUTH_TOKEN = 'ag_default_token';

// Project Manager State
let activeProject = null; // { name, path }
let antigravityProcess = null; // child_process handle
...
async function discoverCDP() {
    ...
    for (const port of PORTS) {
        try {
            ...
            // Find all workbench targets
            const workbenches = list.filter(t => t.type === 'page' && (t.url?.includes('workbench.html') || (t.title && t.title.includes('workbench'))));

            if (workbenches.length > 0) {
                // If there are multiple workbenches, try to find the one that doesn't say "Task" (usually internal)
                // or just pick the first standard one.
                let target = workbenches.find(w => !w.title.includes('Task')) || workbenches[0];

                // Hack: If we see a window that is actively focused (has empty url in some CDP versions or specific title format)
                // Let's just use the last one in the list, as it's often the most recently opened
                target = workbenches[workbenches.length - 1];

                console.log('Found multiple workbenches. Selected target:', target.title);
                return { port, url: target.webSocketDebuggerUrl };
            }

            // Priority 2: Jetski/Launchpad (Fallback)
            const jetski = list.find(t => t.type === 'page' && (t.url?.includes('jetski') || t.title === 'Launchpad'));
...
// Capture chat snapshot — returns STRUCTURED message data (not raw HTML)
async function captureSnapshot(cdp) {
    const CAPTURE_SCRIPT = `(async () => {
        let chatRoot = document.getElementById('conversation') || 
        ...
        const scrollContainer = chatRoot.querySelector('.overflow-y-auto, [data-scroll-area]') || chatRoot;
        const scrollInfo = {
        ...
        let workspaceName = '';
        try { workspaceName = document.title.replace(' - Antigravity', '').replace(' - Task', '').trim(); } catch(e) {}
        const workspaceChats = [];
        try {
            const allBodyButtons = Array.from(document.body.querySelectorAll('button, a, [role="button"]'));
            const chatElements = allBodyButtons.filter(b => {
                const text = (b.innerText || '').trim();
                return text.length > 5 && 
                       (b.className.includes('w-full') || b.className.includes('grow') || b.closest('.sidebar') || b.closest('[class*="sidebar"]')) &&
                       !text.includes('Run') && !text.includes('Reject') && !b.querySelector('svg.lucide-arrow-right');
            });
            const chatTitles = new Set();
            chatElements.forEach(el => chatTitles.add((el.innerText || '').trim()));
            chatTitles.forEach(t => { if(t) workspaceChats.push(t); });
        } catch(e) {}
        
        // Also keep original HTML for backward compat and fallback rendering
        const clone = chatRoot.cloneNode(true);
        // Convert local images to base64 in clone
        ...
        try {
            ...
            hasStopButton = !!(cancelBtn?.offsetParent || stopBtn?.offsetParent);
            activeGeneration = hasStopButton;
            const allBtns = document.querySelectorAll('button');
            for (const btn of allBtns) {
                const text = (btn.innerText || '').trim().toLowerCase();
                if ((text.includes('continue') || text.includes('proceed') || text.includes('resume')) && btn.offsetParent) {
                    hasContinueButton = true; break;
                }
            }
        } catch(e) {}
    ...
    for (const ctx of cdp.contexts) {
        try {
            ...
            if (result.result && result.result.value) {
                ...
                if (val.error) { /* skip */ } else { return val; }
            }
        } catch (e) {
...
// Inject files into Antigravity
async function injectFiles(cdp, files) {
    if (!files || files.length === 0) return { ok: true, reason: 'no_files' };

    try {
        const tmpDir = join(os.tmpdir(), 'ag_phone_uploads');
        ...
        for (const ctx of cdp.contexts) {
            try {
                // Find input node in this context
                const EXP = `(() => {
                ...
                if (res.result && res.result.value) {
                    // It has the element! Now we need the Node ID
                    const docRes = await cdp.call("DOM.getDocument", {});
                    const queryRes = await cdp.call("DOM.querySelector", {
        ...
        // Clean up files after a delay to ensure Antigravity reads them
        setTimeout(() => {
        ...
        return { ok: injected, method: 'cdp_upload' };
    } catch (err) {
...
async function clickElement(cdp, { selector, index, textContent, isCommandAction }) {
    const safeText = JSON.stringify(textContent || '');
    const isCommand = isCommandAction ? 'true' : 'false';
    ...
    const EXP = `(async () => {
        try {
            ...
            // Special handling for Action buttons (Run / Reject / Retry)
            if (isCmd && filterText) {
                const isRun = /^Run/i.test(filterText);
                const isReject = /^Reject/i.test(filterText);
                const isRetry = /^Retry/i.test(filterText);
                
                let matches = [];
                
                // Specific logic for Retry: might just be an icon button or have aria-label
                if (isRetry) {
                    const allButtons = Array.from(root.querySelectorAll('button, [role="button"]'));
                    matches = allButtons.filter(btn => {
                        if (btn.offsetParent === null) return false; // Hidden
                        const txt = (btn.innerText || btn.textContent || '').trim().toLowerCase();
                        const title = (btn.getAttribute('title') || '').toLowerCase();
                        const aria = (btn.getAttribute('aria-label') || '').toLowerCase();
                        
                        return txt.includes('retry') || title.includes('retry') || aria.includes('retry') || btn.querySelector('svg.codicon-refresh') || btn.querySelector('svg.lucide-refresh-cw');
                    });
                } else {
                    // Standard logic for Run/Reject
                    const allButtons = Array.from(root.querySelectorAll('button, [role="button"]'));
                    matches = allButtons.filter(el => {
                ...
                // Click the correct index (or the last one if we expect it to be the most recent message)
                if (matches.length > 0) {
                    // Usually we want the *last* command action if index is way off
                    const targetIdx = Math.min(${index}, matches.length - 1);
                    const target = matches[targetIdx];
                    
                    if (target.focus) target.focus();
                    target.click();
                    return { success: true, method: 'command_action', tag: target.tagName, found: matches.length };
                }
            }

            // Normal Element Click strategy
...
async function startNewChat(cdp) {
    const EXP = `(async () => {
        try {
            // First try to find "Recent in Workspace" or "Workspace Chats" label, and click the "+" button near it
            const allSpans = Array.from(document.querySelectorAll('span, p, div, h1, h2, h3, h4'));
            const workspaceTitle = allSpans.find(el => {
                const text = (el.innerText || '').toLowerCase().trim();
                return text === 'recent in workspace' || text === 'workspace chats';
            });
            
            if (workspaceTitle) {
                // Look around this title for a button containing a plus icon or "New Chat" text
                let container = workspaceTitle.parentElement;
                while (container && container !== document.body) {
                    const btns = Array.from(container.querySelectorAll('button'));
                    const target = btns.find(b => {
                        const text = (b.innerText || '').toLowerCase();
                        return (text.includes('new') || b.querySelector('svg.lucide-plus') || b.querySelector('svg[class*="plus"]'));
                    });
                    
                    if (target) {
                        target.click();
                        return { success: true, method: 'workspace_plus_button' };
                    }
                    container = container.parentElement;
                }
            }

            // Priority 2: Exact selector from user (data-tooltip-id="new-conversation-tooltip")
...
async function createServer() {
    ...
    // Auth Middleware (Protects all subsequent API endpoints)
    app.use((req, res, next) => {
        ...
        // Return 401 for all protected endpoints. 
        // Our client-side fetchWithAuth intercepts 401 and redirects to /login.html
    ...
    app.get('/projects', (req, res) => {
        const projectsDir = appConfig.projectsDir;
        if (!projectsDir || !fs.existsSync(projectsDir)) {
            return res.json({ 
                error: 'Projects directory not configured. Run setup.sh first.',
                projects: [],
                configured: false
            });
        }
        ...
        try {
            ...
            const projects = entries
                ...
                .map(e => {
                    const fullPath = join(projectsDir, e.name);
                    ...
                    const hasPackageJson = fs.existsSync(join(fullPath, 'package.json'));
                    const hasGit = fs.existsSync(join(fullPath, '.git'));
                    const hasCargo = fs.existsSync(join(fullPath, 'Cargo.toml'));
                    const hasPyProject = fs.existsSync(join(fullPath, 'pyproject.toml')) || 
                                         fs.existsSync(join(fullPath, 'setup.py'));
                    ...
                    return {
                        ...
                        type,
                        hasGit,
                        isActive: activeProject?.path === fullPath
                    };
                })
                .sort((a, b) => {
                    // Active project first, then by last modified
    ...
    app.get('/active-project', (req, res) => {
        res.json({
            activeProject: activeProject,
            cdpConnected: cdpConnection?.ws?.readyState === 1,
            configured: !!appConfig.projectsDir
        });
    });
    ...
    app.post('/send', async (req, res) => {
        ...
        // Handle File Injection if there are files
        if (files && files.length > 0) {
            const fileRes = await injectFiles(cdpConnection, files);
            if (!fileRes.ok) {
                console.warn('Failed to inject files:', fileRes);
            }
            // Add a small delay so files are attached before text is injected
            await new Promise(r => setTimeout(r, 500));
        }

        let result = { ok: true, method: 'files_only' };
        if (message) {
            result = await injectMessage(cdpConnection, message);
        }
...
async function main() {
    ...
    try {
        ...
        app.get('/chat-history', async (req, res) => {
            ...
            await closeHistory(cdpConnection);
            res.json(result);
        });
